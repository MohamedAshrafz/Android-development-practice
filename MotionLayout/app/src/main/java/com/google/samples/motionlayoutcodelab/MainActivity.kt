...
class MainViewHolder(val cardView: CardView) : RecyclerView.ViewHolder(cardView) {
    ...
    fun bind(step: Step) {
        ...
        val color = if (step.highlight) {
            ResourcesCompat.getColor(context.resources, R.color.secondaryLightColor, context.theme)
        } else {
            ResourcesCompat.getColor(context.resources, R.color.primaryTextColor, context.theme)
        }
