...
class RemindersLocalRepositoryTest {
    ...
    fun getReminder_dataNotFoundError() = runBlocking {
        ...
        assertThat(loadedReminder is Result.Error, `is`(true))
        loadedReminder as Result.Error
        assertThat(loadedReminder.message, `is`("Reminder not found!"))
    }

    // deleting all reminder then testing getting an item with its id
    @Test
    fun deleteAllRemindersAndGetReminder_dataNotFoundError() = runBlocking {
        // making a new reminder
        val reminder = ReminderDTO(
            "reminder4",
            "this is the first test reminder",
        ...
        // adding the reminder to the repository
        remindersLocalRepository.saveReminder(reminder)

        // GIVEN - deleting all reminders from the repository
